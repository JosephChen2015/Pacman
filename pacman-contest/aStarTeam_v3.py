...
class OffensiveReflexAgent(ReflexCaptureAgent):
    ...
    def isOpponentDefenderFinishScare(self, defender):
        if 20 >= defender.scaredTimer > 0: return True
        return False
    ...
    def chooseAction(self, gameState):
        ...
        if gameState.getAgentState(self.index).numCarrying > 15 or self.isOurAttackerReturnInTiem(gameState):
            return self.aStarSearch(gameState, nearestMiddleLine, self.avoidOpponentDefenderHeuristic)

        # when to escape
        if len(defenders) != 0:
            for defender in defenders:
                if self.getMazeDistance(defender.getPosition(), gameState.getAgentState(self.index).getPosition()) < 5 and \
