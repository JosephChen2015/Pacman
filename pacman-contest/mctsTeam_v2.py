...
class ReflexCaptureAgent(CaptureAgent):
    ...
    def getNearestOpponentAttacker(self, gameState):
        ...
        if len(attackers) != 0:
            for a in attackers:
                distance.append(
                    self.getMazeDistance(gameState.getAgentState(self.index).getPosition(), a.getPosition()))
    ...
    def getDirectlyAvailableFood(self, gameState):
        ...
        for f in self.getFood(gameState).asList():
            ...
            for foodFringe in foodFringes:
                if (foodFringe[0], foodFringe[1]) not in self.walls and not self.hasFood(gameState, foodFringe[0],
                                                                                         foodFringe[1]):
                    count = count + 1
            if count > 1:
    ...
    def getNearestDirectlyAvailableFood(self, gameState):
        distance = []
        nearestAvailableFood = []
        directlyAvailableFood = self.getDirectlyAvailableFood(gameState)
...
class OffensiveReflexAgent(ReflexCaptureAgent):
    ...
    def chooseAction(self, gameState):
        ...
        nearestFood = self.getNearestFoodToEat(gameState)
        nearestDirectlyAvailableFood = self.getNearestDirectlyAvailableFood(gameState)
        nearestMiddleLine = self.getNearestMiddleLine(gameState)
        defenders = self.getOpponentDefenders(gameState)
        attackers = self.getOpponentAttackers(gameState)
        ...
        if len(defenders) != 0:
            for defender in defenders:
                if self.getMazeDistance(defender.getPosition(),
                                        gameState.getAgentState(self.index).getPosition()) < 4 and \
                        self.isOpponentDefenderFinishScare(defender):
                    return self.aStarSearch(gameState, nearestMiddleLine, self.avoidOpponentDefenderHeuristic)

        # when to search for the nearest directly available food
        if len(self.getDirectlyAvailableFood(gameState)) != 0:
            if len(defenders) != 0:
                for defender in defenders:
        ...
        # when to search for the nearest food
        if len(defenders) != 0:
            for defender in defenders:
                if self.isOpponentDefenderStartScare(defender) or \
                        self.getMazeDistance(defender.getPosition(),
                                             gameState.getAgentState(self.index).getPosition()) > 10:
                    return self.aStarSearch(gameState, nearestFood, self.avoidOpponentDefenderHeuristic)

        # when to search for the nearest capsule
        if nearestCapsule != None and len(self.getDirectlyAvailableFood(gameState)) == 0:
            if len(defenders) != 0:
                for defender in defenders:
        ...
        # Guarantee the carrying food to be returned
        if len(self.getFood(gameState).asList()) == 0 or \
                (nearestCapsule == None and gameState.getAgentState(self.index).numCarrying != 0 and len(
                    defenders) != 0):
            return self.aStarSearch(gameState, nearestMiddleLine, self.avoidOpponentDefenderHeuristic)
...
class DefensiveReflexAgent(ReflexCaptureAgent):
    ...
    def setNumFoodToDefend(self, gameState, opponent):
        if self.getPreviousObservation() != None and \
                gameState.getAgentState(opponent).numReturned > self.getPreviousObservation().getAgentState(
            opponent).numReturned:
            self.numFoodToDefend -= (
                        gameState.getAgentState(opponent).numReturned - self.getPreviousObservation().getAgentState(
                    opponent).numReturned)
    ...
    def randomMove(self, gameState, previousAction):
        actions = gameState.getLegalActions(self.index)
        actions.remove(Directions.STOP)
        if not previousAction == None and len(actions) > 1 and Directions.REVERSE[previousAction] in actions:
            preAction = Directions.REVERSE[previousAction]
    ...
    def simulate(self, gameState, gamma):
        ...
        while time < limit:
            ...
            # randomly move
            gameState, previousAction = self.randomMove(gameState, previousAction)
            ...
            if gameState.getAgentState(self.index).scaredTimer > 0:
                if len(attackers) != 0:
                    for attacker in attackers:
                        if self.getMazeDistance(currentPosition, attacker.getPosition()) < 2:
                            reward = (100000) * math.pow(gamma, time)
                            return reward
                        elif 2 <= self.getMazeDistance(currentPosition, attacker.getPosition()) < 3:
                            reward = (10000) * math.pow(gamma, time)
                            return reward
                        elif 3 < self.getMazeDistance(currentPosition, nearestAttacker.getPosition()):
                            reward = (100) * math.pow(gamma, time)
                            return reward
            ...
            if gameState.getAgentState(self.index).scaredTimer == 0:
                if len(attackers) != 0:
                    if nearestAttacker != None:
                        if self.getMazeDistance(currentPosition, nearestAttacker.getPosition()) < 2:
                            reward = (100000) * math.pow(gamma, time)
                            return reward
                        elif 2 <= self.getMazeDistance(currentPosition, nearestAttacker.getPosition()) < 3:
                            reward = (10000) * math.pow(gamma, time)
                            return reward
                        elif 3 < self.getMazeDistance(currentPosition, nearestAttacker.getPosition()):
                            reward = (100) * math.pow(gamma, time)
    ...
    def mctsSearch(self, gameState):
        ...
        while time < limit:
            ...
            for a in actConfig.keys():
                value, times = actConfig[a]
                if times == 0:
                    action = a
                    break
                elif time >= 3:
                    v = value * math.pow(gamma, times)
                    if v > maxValue:
                        maxValue = v
                        action = a
            value, times = actConfig[action]
            totalReward = value * times
            ...
            if action in legalActions:
                ...
                successor = gameState
            times += 1
            totalReward += self.simulate(successor, gamma)

            value = totalReward / times
            actConfig[action] = [value, times]

        maxValue = -999999
        for a in actConfig.keys():
            value, times = actConfig[a]
            if value >= maxValue and a in legalActions:
                maxValue = value
                bestAction = a
        print(maxValue)
        ...
        if abs(maxValue) > 100:
            ...
            nearestAttacker = self.getNearestOpponentAttacker(gameState)
            nearestMiddleLine = self.getNearestMiddleLine(gameState)
            nearestFood = self.getNearestFoodToDefend(gameState)
            ...
            if gameState.getAgentState(self.index).getPosition() == self.nearestEatenFood:
                if nearestFood != None:
                    return self.aStarSearch(gameState, nearestFood, self.nullHeuristic)
            ...
            if len(self.getFoodYouAreDefending(self.getCurrentObservation()).asList()) < self.numFoodToDefend:
                if self.getPreviousObservation() != None and len(
                        self.getFoodYouAreDefending(self.getCurrentObservation()).asList()) < \
    ...
    def chooseAction(self, gameState):
        return self.mctsSearch(gameState)